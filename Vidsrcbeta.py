...
def search_imdb(search_query):
    ...
    try:
        ...
        for item in soup.select('li.ipc-metadata-list-summary-item'):
            ...
            type_tag = item.select('span.ipc-metadata-list-summary-item__li')[1] if len(item.select('span.ipc-metadata-list-summary-item__li')) > 1 else None
            ...
            if title_tag and year_tag:
                ...
                if imdb_id:
                    results.append({'title': title, 'year': year, 'type': result_type, 'imdb_id': imdb_id.group(1)})
        return results
...
async def handle_title(update: telegram.Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text("Enter the number of the result you want to see the IMDb ID for:")
    return SELECTION

async def handle_selection(update: telegram.Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        if 1 <= selection <= 7:
            ...
            if selected_result['type'] == "TV Series":
                await update.message.reply_text(reply + "Enter the number of the season you want to watch:")
                return SEASON
            else:
                movie_link = f"https://vidsrc.to/embed/movie/{selected_result['imdb_id']}"
...
async def handle_season(update: telegram.Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
...
async def handle_episode(update: telegram.Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        episode = int(update.message.text)
        selected_result = context.user_data['selected_result']
        season = context.user_data['season']
        movie_link = f"https://vidsrc.to/embed/tv/{selected_result['imdb_id']}/{season}/{episode}"
